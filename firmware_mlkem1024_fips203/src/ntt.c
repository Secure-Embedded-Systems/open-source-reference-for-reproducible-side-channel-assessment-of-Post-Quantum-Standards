...
static int16_t fqmul(int16_t a, int16_t b) {
  int16_t res;
      ...
      /* SCA_MONT_K does (a*b) * R^-1 mod Q in a single ISE instruction. */
      uint32_t r;
      SCA_MONT_K(r, (uint32_t)(int32_t)a, (uint32_t)(int32_t)b);
      res = (int16_t)(int32_t)r;
  #else
...
void invntt(int16_t r[256]) {
    ...
    for (len = 2; len <= 128; len <<= 1) {
        for (start = 0; start < 256; start = j + len) {
            ...
            for (j = start; j < start + len; j++) {
              ...
              asm volatile(
                  "add   %[sum],  %[rj],   %[rlen]\n\t"
                  ".insn r 0x7b, 0x01, 0x0, %[sum], %[sum], x0\n\t"
                  "sub   %[diff], %[rlen], %[rj]\n\t"
                  "mul   %[prod], %[z],    %[diff]\n\t"
                  ".insn r 0x7b, 0x01, 0x2, %[prod], %[prod], x0\n\t"
                  : [sum]  "=&r"(sum),
                  [diff] "=&r"(diff),
                  [prod] "=&r"(prod)
                  : [rj]   "r"(rj),
    ...
    for (j = 0; j < 256; j++) {
      ...
      asm volatile(
          "mul    %[prod], %[x], %[f]\n\t"
          ".insn r 0x7b, 0x1, 0x2, %[out], %[prod], x0\n\t"
...
{
  /* Always uses fqmul; whether fqmul itself is hardware-accelerated is
   * controlled by ENABLE_KYBER_MONTG. */
  r[0]  = fqmul(a[1], b[1]);
  r[0]  = fqmul(r[0], zeta);
  r[0] += fqmul(a[0], b[0]);
  r[1]  = fqmul(a[0], b[1]);
