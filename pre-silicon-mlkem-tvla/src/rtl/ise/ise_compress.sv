...
// ============================================================================
//
// File        : ise_compress.sv
// Description : Compress & rejection-sampling ISE (Group 4).
//               Implements COMPRESS steps 1-4 and REJ_UNIFORM.
//
// Compress_d(x) = round((2^d / Q) * x) mod 2^d   for Q = 3329
//   Step 1 (d=1):  round(x * 2     / 3329) & 0x1
//   Step 2 (d=4):  round(x * 16    / 3329) & 0xF
//   Step 3 (d=5):  round(x * 32    / 3329) & 0x1F
//   Step 4 (d=10): round(x * 1024  / 3329) & 0x3FF
//
// Implementation uses multiply-shift approximation (no hardware divider):
//   u = x + ((x >> 15) & 3329)    // conditional add Q for negative input
//   y = (u << d) + 1664           // scale and round
//   p = y * 80635                 // multiply by reciprocal approximation
//   result = (p >> 28) & mask     // shift and mask
//
// This avoids synthesizing a ~4000 LUT combinational divider.
// The constant 80635 = ceil(2^28 / 3329) is the reciprocal approximation.
//
// REJ_UNIFORM: Extract 12-bit candidate from rs1[11:0].
//   If < 3329, result = candidate; else result = 0xFFFFFFFF (reject flag).
//
// ============================================================================
