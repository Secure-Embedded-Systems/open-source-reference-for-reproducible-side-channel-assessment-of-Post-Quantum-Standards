// ============================================================================
// SCA_Pro_PQC — ISE Masking Primitives
// SECMUL_STEP/LATCH/REUSE, MASK_REFRESH, AND_BLIND/LATCH/REUSE, MASKED_XOR
//
// DOM (Domain-Oriented Masking) Security Model:
// ============================================================================
//
// ** SECMUL_LATCH / SECMUL_REUSE ** — Paired DOM cross-term gadget
//   Implements the Gross-Mangard arithmetic-DOM "+r / -r" cancellation pattern
//   that the original SECMUL_STEP could not (independent PRNG per call leaks
//   residual mask noise on recombination).
//
//     SECMUL_LATCH(a0, b1) → (a0*b1 + r) mod q ; capture r into r_latched
//     SECMUL_REUSE(a1, b0) → (a1*b0 - r) mod q ; uses captured r, no PRNG advance
//
//   Software protocol for full first-order DOM multiplication of (a0+a1)(b0+b1):
//     u00 = MONT_K   (a0, b0)              ; same-share, no DOM needed
//     u11 = MONT_K   (a1, b1)              ; same-share
//     v01 = SECMUL_LATCH (a0, b1)          ; +r blinding, latch r
//     v10 = SECMUL_REUSE (a1, b0)          ; -r blinding, uses same r
//     share0 = (u00 + v01) mod q
//     share1 = (u11 + v10) mod q
//     ; share0 + share1 = a*b mod q   (KAT-correct)
//     ; each individual intermediate (v01, v10) is r-blinded   (probing-secure)
//
//   Hardware atomicity contract:
//     The pair must execute back-to-back without an intervening PRNG-consuming
//     op (which would advance r_latched). Bare-metal firmware that disables
//     interrupts around masked routines satisfies this.
//
//   funct7[4] (q_sel_i) selects Q:
//     0 = Q_KEM  = 3329    (16-bit operands, Montgomery reduction)
//     1 = Q_DSA  = 8380417 (23-bit operands, Barrett reduction)
//
// ** AND_LATCH / AND_REUSE ** — Paired DOM-AND for Keccak chi
//     AND_LATCH (a0, b1) → (a0 & b1) ^ r ; capture r
//     AND_REUSE (a1, b0) → (a1 & b0) ^ r ; same r
//   Boolean cancellation on XOR-recombination:
//     share0 = (a0 & b0) ^ (a0 & b1) ^ r
//     share1 = (a1 & b1) ^ (a1 & b0) ^ r
//     share0 ^ share1 = a & b   (r cancels)
//
// ** SECMUL_STEP / MASKED_AND (legacy) ** — kept for area / micro-bench.
//   Still useful when only single-share blinding is wanted (mask-refresh,
//   randomized comparison, etc.). NOT a complete DOM gadget on their own.
//
// ** AND_BLIND (opcode: MASKED_AND) ** — Single share-pair AND with blinding
//   result = (rs1 & rs2) ^ prng_random
//   This is ONE building block of a DOM-AND gadget for Keccak Chi masking.
//   For a full first-order DOM-AND of masked values, software must call 4 times:
//     Call 1: AND_BLIND(a0, b0) → p00  (inner, randomized)
//     Call 2: AND_BLIND(a0, b1) → p01  (cross-term, fresh random)
//     Call 3: AND_BLIND(a1, b0) → p10  (cross-term, fresh random)
//     Call 4: AND_BLIND(a1, b1) → p11  (inner, randomized)
//   Recombine using MASKED_XOR: share0 = p00 ^ p01, share1 = p10 ^ p11
//   Each call returns (rs1 & rs2) ^ fresh_random, and the PRNG is consumed
//   to ensure independent randomness per invocation.
//
// ** MASK_REFRESH ** — Re-randomize a single share
//   q_sel_i=0: Boolean refresh: result = rs1 ^ prng_random
//   q_sel_i=1: Arithmetic refresh: result = (rs1 + prng_random[11:0]) mod Q_KEM
//   The complementary share must be adjusted by software accordingly:
//     Boolean: other_share ^= prng_random  (use MASKED_XOR with GET_RANDOM)
//     Arithmetic: other_share -= prng_random[11:0] mod Q_KEM
//
// ** MASKED_XOR ** — Linear XOR (no randomness needed)
//   result = rs1 ^ rs2
//   Used to combine shares in the DOM recombination step.
//
// ============================================================================
...
module ise_mask_ops (
  ...
  //
  // Hardware atomicity contract (TCHES-reviewer fix, Stage-1 round 2):
  ...
  // subsequent REUSE traps.  TCHES Round-2 reviewer fix: GET_RANDOM is now
  ...
  // This sharing was a TCHES round-1 reviewer fix: doubling the Barrett-D
  ...
  // TCHES Round-2 silicon-correctness fix: previous version multiplied
  ...
  // TCHES round-2 fix: previous Barrett-D path produced output without the
